...
async register(username, group = '', password) {
    try {
        ...
        // УДАЛИТЕ ЭТУ ЧАСТЬ - НЕ СТАВИМ ПОЛЬЗОВАТЕЛЯ В ТЕКУЩЕГО
        // НЕ АВТОРИЗУЕМ АВТОМАТИЧЕСКИ
...
async saveUserStats(stats) {
    ...
    try {
        ...
        // ВАЖНО: Добавляем заголовок Cache-Control
        const response = await fetch(`${this.supabaseUrl}/rest/v1/users?id=eq.${this.currentUser.id}`, {
            method: 'PATCH',
            headers: {
                'apikey': this.supabaseKey,
                'Authorization': `Bearer ${this.supabaseKey}`,
